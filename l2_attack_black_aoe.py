...
class BlackBoxL2:
    ...
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
        ...
        self.timg1 = tf.Variable(np.zeros(single_shape), dtype=tf.float32) 
        self.timg2 = tf.Variable(np.zeros(single_shape), dtype=tf.float32)  
        self.timg3 = tf.Variable(np.zeros(single_shape), dtype=tf.float32) 
        ...
        self.assign_timg3 = tf.placeholder(tf.float32, single_shape)
        ...
        #self.timg2_grgb = tf.image.grayscale_to_rgb(tf.tanh(self.timg2)/2+ 0.5)
        # print('newimg grgb',self.newimg_grgb.shape)
        # self.loss3= tf.reduce_sum((self.newimg_grgb-self.timg2_grgb), [1,2,3])
        # print('check if two tensors are the same weird ======', self.weird)
        else:
        ...
        if self.htype == "phash":
            ...
            if use_tanh:
                ...
                self.output2_t1 = model.predict1(self.newimg, self.timg1, self.method)
                self.output2_t2 = model.predict1(self.newimg, self.timg2, self.method)
                self.output2_t3 = model.predict1(self.newimg, self.timg3, self.method)
        ...
        elif self.htype == "blockhash":
            if use_tanh:
                #self.output2 = model.predict1(self.newimg, tf.tanh(self.timg) / 2, self.method)
                self.output2_t0 = model.predict2(self.newimg, tf.tanh(self.timg) / 2)
                self.output2_t1 = model.predict2(self.newimg, self.timg1)
                self.output2_t2 = model.predict2(self.newimg, self.timg2)
                self.output2_t3 = model.predict2(self.newimg, self.timg3)
 ...
 # for placeholder purpose
        ...
        # if use_tanh:
        #     #self.l2dist = tf.reduce_sum(tf.square(self.newimg2-tf.tanh(self.timg2)/2), [1,2,3])
        #     self.l2dist_original = tf.reduce_sum(tf.square(self.newimg-tf.tanh(self.timg)/2), [1,2,3])
        #     #self.l2dist_trans = tf.reduce_sum(tf.square(self.newimg-tf.tanh(self.timg2)/2), [1,2,3])

        # else:
        #     self.l2dist = tf.reduce_sum(tf.square(self.newimg - self.timg), [1,2,3])
            #self.l2dist = tf.reduce_sum(tf.square(self.newimg2 - self.timg2), [1,2,3])
        # self.output2_t0 = self.sess.run(self.output2_t0)
        # self.output2_t1 = self.sess.run(self.output2_t1)
        # self.output2_t2 = self.sess.run(self.output2_t2)
        # self.output2_t3 = self.sess.run(self.output2_t3)
        # self.output2_t4 = self.sess.run(self.output2_t4)
        self.output2 = (1/4 * (self.output2_t0 + self.output2_t1 + self.output2_t2+self.output2_t3))
        ...
        if self.dist_metrics == "pdist": 
            ...
            # print('if tensor 0 equals tensor 2 ', self.input0 == self.input2)
            # print('previous input shape====== ', self.input0)
            # print('if two tensors are the same ----------------', tf.math.equal(self.input0, self.input2))
            ...
            # print('previous input shape============ ', self.input0)
            # print('input0 shape with -1,', self.input0)
            self.input1 = tf.reshape(self.input1, tf.concat([[-1], tf.shape(self.input1)[-3:]], axis=0))
            # self.input2 = tf.reshape(self.input2, tf.concat([[-1], tf.shape(self.input2)[-3:]], axis=0))
            # print('previous input shape============ ', self.input1)
            # NHWC to NCHW
       ...
       # loss1 = self.output2
        ...
        # self.loss3 = self.loss3 * self.const
        '''# 修改 这里hash loss = loss1 乘上了系数'''
...
#        self.real1 = tf.maximum(0.0, self.real1 - self.other1 +self.CONFIDENCE)
        ...
        self.setup.append(self.timg3.assign(self.assign_timg3))
    ...
    def attack_batch(self, gray_img, img, gray_img_t1, gray_img_t2, gray_img_t3, img_id):
        ...
        if len(gray_img_t1.shape) == 4:
            gray_img_t1 = gray_img_t1[0]
            gray_img_t2 = gray_img_t2[0]
            gray_img_t3 = gray_img_t3[0]
        ...
        gray_img_t1 = gray_img_t1.astype(np.float32)
        gray_img_t2 = gray_img_t2.astype(np.float32)
        gray_img_t3 = gray_img_t3.astype(np.float32)
        ...
        for outer_step in range(binary_search_step):
            ...
            self.sess.run(self.setup, {self.assign_timg: gray_img, self.assign_timg1: gray_img_t1, self.assign_timg2: gray_img_t2, self.assign_timg3: gray_img_t3, self.assign_const: CONST})
