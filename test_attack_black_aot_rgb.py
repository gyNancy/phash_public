...
def generate_aot(gray_inputs, ratio):
    ...
    gray_inputs1 = resize(gray_inputs, (height, int(width * ratio)),anti_aliasing=True)
...
def generate_transformation(all_gray_inputs, i, ratio):
    ...
    rotation_array= imgs_tform2[int ((height*0.05)/ 2): int(height - (height*0.05)/ 2), int ((height*0.05)/ 2):int(width - (width*0.05)/ 2), :] #0.05
...
# cropped_array7 = all_gray_inputs[30:269, 30:269] 
    ...
    # aot_image = gen_image(aot_array)
    # aot_image.save("aot_img{}.png".format(i))
...
def main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        # for i in start_idxs:
            ...
            inputs_img_original = gen_image(inputs_original)
            ...
            if args["htype"] == "phash":
                ...
                hash_differences_original = imagehash.phash(inputs_img_original, args['bits'], args['factor']) - imagehash.phash(adv_img, args['bits'], args['factor'])
            elif args["htype"] == "blockhash":
                ...
                hash_differences_original = sum(1 for i, j in zip(robusthash.blockhash(inputs_img_original), robusthash.blockhash(adv_img)) if i != j)
            ...
            if L3 == True: 
                ...
                if success:
                    ...
                    hash_total_original += hash_differences_original
                    ...
                    print('saving for successful attack current', suffix)
                ...
                if args["htype"] == "phash":
                    ...
                    hash_differences_current_original = imagehash.phash(inputs_img_original, args['bits'], args['factor']) - imagehash.phash(adv_current_img, args['bits'], args['factor'])
                elif args["htype"] == "blockhash":
                    if inputs_img.mode == '1' or inputs_img.mode == 'L' or inputs_img.mode == 'P':
                        im_original = inputs_img.convert('RGB')
                        im_adver_current = adv_current_img.convert('RGB')
                        im_original2 = inputs_img_original.convert('RGB')
                    else:
                        im_original = inputs_img
                        im_adver_current = adv_current_img
                        im_original2 = inputs_img_original
                    ...
                    print('robust hash differences', hash_differences)
                    hash_differences_current_original = sum(1 for i, j in zip(robusthash.blockhash(im_original2), robusthash.blockhash(im_adver_current)) if i != j)
                total_iterations += first_iteration
                hash_total2 += hash_differences_current
                l2_total2 += l2_distortion_current
                pdistance_total +=  distance2[0]
                hash_total_original += hash_differences_current_original
            ...
            print("overal average hash original ", hash_total_original / (img_no-args["start_idx"]))
            sys.stdout.flush()  
...
if __name__ == "__main__":
    ...
    if args['dataset'] == "mnist":
        args['use_tanh'] = True
    # img6 is for small batch
    # if args["dataset"] == "face":
    #         start_idx = 1
    # else:
    #     start_idx = 0 
    args['load_ckpt'] = './{}/best_modifier_img{}.npy'.format(args['save_ckpts'], args["start_idx"])
